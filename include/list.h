...
#include "node.h"    // Include node structure definition
#include <stdbool.h> // for bool type
//_Bool is the primitive and ^this is a macro for it

/*
    A List is container that owns multiple Nodes

    Ownership rules:
    - List is heap allocated with list_create
    - Destoying list (with list_destroy) frees all owned Nodes
    - Owns all Nodes reachable from its head
*/
typedef struct
{
    Node *head; // Pointer to the first node in the list (or NULL if empty)
} List;
...
/*
    Frees memory allocated for a List and all its owned Nodes
...
/*
    Removes the Node first encountered that contains the given data.
